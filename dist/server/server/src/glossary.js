...
function parse(raw, slug, updatedAt) {
    const match = raw.match(/^---\n([\s\S]*?)\n---\n([\s\S]*)$/);
    if (!match)
        return { slug, term: slug, aliases: [], content: raw.trim(), updatedAt };
...
export async function upsertGlossaryTerm(input, existingSlug) {
    await ensureDir();
    ...
    if (!term)
        throw new Error("GLOSSARY_WRITE_FAILED");
    return term;
}
export async function deleteGlossaryTerm(slug) {
    await ensureDir();
    try {
