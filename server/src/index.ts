...
import {
  createModule, deleteModule, getState, updateCheck,
...
app.patch("/api/checks/:moduleId", async (req, res, next) => {
  try {
    ...
    if (input.checked) await recordActivity();  // ← добавить
    res.json(checks);
  } catch (error) { next(error); }
});
...
app.get("/api/notes/:lessonId", async (req, res, next) => {
  try { res.json({ content: await getNote(req.params.lessonId) }); }
  catch (e) { next(e); }
});
...
app.get("/api/review/:lessonId", async (req, res, next) => {
  try { res.json(await getReview(req.params.lessonId)); }
  catch (e) { next(e); }
});
...
app.get("/api/export", async (_req, res, next) => {
  try { res.json(await exportState()); }
  catch (e) { next(e); }
});

app.post("/api/import", async (req, res, next) => {
  try { await importState(req.body); res.json({ ok: true }); }
  catch (e) { next(e); }
});
